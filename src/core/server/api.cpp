...
void outDeviceNode(HttpResp* resp, trivial::cbMySqlDevice* deviceNode, int node_num,
                   int device_num,int posx=100,int posy=200) {
  // int posx = 100;
...
// op的id op的输入槽个数 op_num不用管
void outOpNode(int node_num, int input_num, HttpResp* resp, int op_num,int posx=100,int posy=300) {
  // int posx = 100 + 1 * 250;
...
//
void outFinNode(int device_num, int op_num, HttpResp* resp,int posx,int posy) {
  // int posx = 100 + 2 * 250;
...
//nodeleftnum Left节点 noderightnum op节点 nowinputsum现在op哪一个位置
...
//nodeleftnum Op节点 nowinputsnum现在Op哪一个位置
